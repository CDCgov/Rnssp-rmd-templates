...
params:
  ...
  site:
    ...
    value: "All"
    input: select
    choices: !r stn <- tempfile(fileext ="nca_hosp.rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "text_mining", "skeleton", "nca_hosp.rds"), destfile = stn); dplyr::pull(readRDS(stn), site_name)
    multiple: yes
  age_groups: 
    label: "Age Groups: "
    value: "All Ages"
    input: select
    choices: !r agrp <- tempfile(fileext ="essagegrps.rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "text_mining", "skeleton", "essagegrps.rds"), destfile = agrp); dplyr::pull(readRDS(agrp), display_name)
    multiple: yes
  definition: 
    label: "Syndrome Definition: " 
    value: !r  fl <- tempfile(fileext =".rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "text_mining", "skeleton", "syndef.rds"), destfile = fl); dplyr::pull(head(readRDS(fl), 1), syndrome)
    input: select
    choices: !r fl <- tempfile(fileext =".rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "text_mining", "skeleton", "syndef.rds"), destfile = fl); dplyr::pull(readRDS(fl), syndrome)
  doc_title:
    label: "Title: "
...
# ccdd_category_string <- params$definition
```
...
```{r ESSENCE url build, echo=FALSE, message = FALSE, warning = FALSE, results = 'hide'}

field <- as.character(str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome"))

if(as.character(str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome")) == "CCDD Category"){
  definition_string <- str_replace_all(str_remove_all(tolower(params$definition), "ccdd category: "), " ", "%20")
  definition_type <- "&ccddCategory="
}

if(as.character(str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome")) == "Subsyndrome"){
  definition_string <- str_replace_all(str_remove_all(tolower(params$definition), "subsyndrome: "), " ", "%20")
  definition_type <- "&subsyndrome="
}

if(as.character(str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome")) == "Syndrome"){
  definition_string <- str_replace_all(str_remove_all(tolower(params$definition), "syndrome: "), " ", "%20")
  definition_type <- "&syndrome="
}

if(params$site != "All"){
  site_id <- read.csv("site_id_master.csv") %>% filter(site_name == params$site) %>% pull(site_id)
}

if(params$data_source == "Chief Complaint Query Validation"){
  url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails?endDate=", endDate, definition_type, definition_string, "&timeResolution=weekly&percentParam=noPercent&userId=2362&datasource=va_erccdd&aqtTarget=DataDetails&detector=nodetectordetector&startDate=", startDate, "&hasBeenE=", hasBeenE)
}else if(params$data_source == "Facility Location (Full Details)"){
  if(params$age_groups == "All Ages"){
    if(params$site == "All"){
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails?endDate=", endDate, "&percentParam=noPercent&datasource=va_hosp&startDate=", startDate, "&medicalGroupingSystem=essencesyndromes&userId=2362&aqtTarget=DataDetails&ccddCategory=", definition_string, "&geographySystem=hospital&detector=nodetectordetector&timeResolution=daily&hasBeenE=", hasBeenE, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=CCDD")
    } else{
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails?endDate=", endDate, "&percentParam=noPercent&datasource=va_hosp&startDate=", startDate, "&medicalGroupingSystem=essencesyndromes&userId=2362&site=", site_id, "&aqtTarget=DataDetails&ccddCategory=", ccdd_category_string, "&geographySystem=hospital&detector=nodetectordetector&timeResolution=daily&hasBeenE=", hasBeenE, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=CCDD")
    }
  }else {
    ...
    if(params$site == "All"){
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails?endDate=", endDate,
                    "&percentParam=noPercent&datasource=va_hosp&startDate=", startDate, "&medicalGroupingSystem=essencesyndromes&userId=2362&aqtTarget=DataDetails&ccddCategory=", definition_string, "&geographySystem=hospital&detector=nodetectordetector&timeResolution=daily&hasBeenE=", hasBeenE, "&", age_group_string,  "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=CCDD&field=Age")
    } else{
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails?endDate=", endDate,
                    "&percentParam=noPercent&datasource=va_hosp&startDate=", startDate, "&medicalGroupingSystem=essencesyndromes&userId=2362&site=", site_id, "&aqtTarget=DataDetails&ccddCategory=", definition_string, "&geographySystem=hospital&detector=nodetectordetector&timeResolution=daily&hasBeenE=", hasBeenE, "&", age_group_string,  "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=CCDD&field=Age")
    }
  }
}
...
### `r paste("Query Details:", str_squish(str_remove_all(params$definition, "CCDD Category|Syndrome|Subsyndrome|:")))`

```{r query, echo = FALSE, warning = FALSE, message = FALSE}

if (params$age_groups == "All"){
  
  field <- str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome")
  
  query <- readRDS("syndef.rds") %>%
    filter(syndrome == params$definition) %>%
    pull(query_logic)
  ...
  field <- c(str_match(params$definition, pattern = "CCDD Category|Subsyndrome|Syndrome"), as.character(str_match(params$age_group, pattern = "([[:alnum:]]*|\\-|\\s*)*(?=:)")[, 1])) %>%
    unique() 
