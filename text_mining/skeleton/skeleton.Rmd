...
if (nchar(params$complex_query) > 0) {
  ...
  if (params$data_source == "Chief Complaint Query Validation") {
    url <- paste0(
      "https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?endDate=", endDate, "&timeResolution=weekly&percentParam=noPercent", definition_type, definition_string, "&userId=2362&datasource=va_erccdd&aqtTarget=DataDetails&detector=nodetectordetector&startDate=", startDate, "&hasBeenE=", hasBeenE,
...
data <- api_data %>%
  ...
  .[, ccdd := paste(chief_complaint_parsed, discharge_diagnosis)]
```
...
edges <- cc_fcm %>%
  tidytext::tidy() %>%
  slice_max(count, n = 200, with_ties = FALSE) %>%
  select(
    from = document,
    to = term,
    value = count
