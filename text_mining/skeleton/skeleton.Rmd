...
params:
  ...
  age_groups: 
    ...
    multiple: yes
  definition: 
    label: "Definition (CCDD Category, Subsyndrome, or Syndrome): " 
    value: "None (Custom CCDD Query)"
    input: select
    choices: !r definitions <- tibble::add_row(readRDS("syndef_new.rds"), syndrome = "None (Custom CCDD Query)", query_logic = "", .before = 1); dplyr::pull(definitions, syndrome)
  ccdd_query: 
    label: "CCDD Query:"
    value: ""
    input: text
  ...
  complex_query: 
    label: "Complex Query (will override all previous parameters except for dates):"
    value: ""
    input: text
...
if (nchar(params$complex_query) > 0) {
  ...
  if (!grepl("&field=", url)) {
    url <- change_dates(paste0(url, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=CCDD&field=DDParsed&field=CCDDParsed"), start_date = params$start_date, end_date = params$end_date)
  }
...
api_data <- try(get_api_data(url, fromCSV = TRUE),
  silent = TRUE
)
...
data <- api_data %>%
  ...
  .[, dd_parsed_orig := dd_parsed] %>%
  .[, ccdd_parsed_orig := ccdd_parsed] %>%
  relocate(c(linenumber, year, week, date, month, chief_complaint_parsed_orig, dd_parsed_orig, ccdd_parsed_orig)) %>%
  .[, chief_complaint_parsed := vapply(lapply(str_split(chief_complaint_parsed, " "), unique), paste, character(1L), collapse = " ")] %>%
  .[, discharge_diagnosis := gsub(";", " ", dd_parsed)] %>%
  ...
  .[, ccdd := vapply(lapply(str_split(ccdd_parsed, " "), unique), paste, character(1L), collapse = " ")]
```
...
if (nchar(params$complex_query) > 0) {
  ...
  data.frame(Fields = fields, Query = c(syndrome_free_text, ccdd_category_free_text)) %>%
    mutate(Query = if_else(Query == "NA", "Not used, see API URL", Query)) %>%
    mutate(Query = str_replace_all(Query, pattern = "\\^", "\\\\^")) %>%
    kable("html") %>%
    kable_styling() %>%
    add_footnote(label = query_url, escape = FALSE, notation = "none") %>%
...
valueBox(n_encounters, icon = "fa-hospital", color = "#EEF9FF")
```
...
valueBox(range, icon = "fa-calendar", color = "#EEF9FF")
```
...
data %>%
  ...
  e_tooltip(
    ...
    formatter = htmlwidgets::JS("
      function(params){
        return('<b>MMWR Week Date:</b> ' + params[0].value[0]) +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params[0].value[1].toLocaleString())
      }
    ")
  ) %>%
  e_legend(show = FALSE)
...
ccngram %>%
  arrange(n) %>%
  e_charts(word) %>%
  e_bar(n, name = "Frequency", color = "#005EAA") %>%
  e_flip_coords() %>%
  e_x_axis(
    name = "Frequency",
    nameLocation = "center",
  ...
  e_tooltip(
    formatter = htmlwidgets::JS("
      function(params){
        return('<b>Term:</b> ' + params.value[1] +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params.value[0].toLocaleString()))
      }
    "),
    textStyle = list(overflow = "break", fontSize = 11)
  ) %>%
  e_legend(show = FALSE) %>%
  e_title("Top 200 Single Term Frequencies of Chief Complaint Parsed") %>%
...
ddngram %>%
  arrange(n) %>%
  mutate(model = paste(code, description, sep = ",")) %>%
  e_charts(code) %>%
  e_bar(n, name = "Frequency", color = "#005EAA", bind = model) %>%
  e_flip_coords() %>%
  e_x_axis(
    name = "Frequency",
    nameLocation = "center",
  ...
  e_tooltip(
    formatter = htmlwidgets::JS("
      function(params){
        ...
        return('<b>Code:</b> ' + params.value[1] +
        '<br /><b>Description:</b> ' + vals[1] +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params.value[0].toLocaleString()))
      }
    "),
    textStyle = list(overflow = "break", fontSize = 11)
  ...
  e_legend(show = FALSE) %>%
  e_title("Top 200 Single Term Frequencies of Discharge Diagnosis Parsed") %>%
  e_grid(containLabel = TRUE)
```
...
cc_bigram_top200 %>%
  ...
  e_tooltip(
    formatter = htmlwidgets::JS("
      function(params){
        return('<b>Bigram:</b> ' + params.value[1] +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params.value[0].toLocaleString()))
      }
    "),
    textStyle = list(overflow = "break", fontSize = 11)
  ...
  e_legend(show = FALSE) %>%
  e_title("Top 200 Bigram Frequencies of Chief Complaint Parsed") %>%
  e_grid(containLabel = TRUE)
```
...
ddngram %>%
  arrange(n) %>%
  mutate(
    code1 = str_wrap(paste(code1, description1, sep = "-"), 90),
    code2 = str_wrap(paste(code2, description2, sep = "-"), 90),
    model = paste(code1, code2, sep = "_"),
    model = gsub("\n", "<br />", model)
  ) %>%
  e_charts(bigram) %>%
  e_bar(n, name = "Frequency", color = "#005EAA", bind = model) %>%
  ...
  e_tooltip(
    formatter = htmlwidgets::JS("
      function(params){
        var vals = params.name.split('_')
        return('<b>Code 1:</b> ' + vals[0] +
        '<br /><b>Code 2:</b> ' + vals[1] +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params.value[0].toLocaleString()))
      }
    "),
    textStyle = list(overflow = "break", fontSize = 11)
  ...
  e_legend(show = FALSE) %>%
  e_title("Top 200 Bigram Frequencies of Discharge Diagnosis Parsed") %>%
  e_grid(containLabel = TRUE)
```
...
ccngram %>%
  ...
  e_tooltip(
    formatter = htmlwidgets::JS("
      function(params){
        return('<b>Trigram:</b> ' + params.value[1] +
        '<br /><b>Frequency:</b> ' + echarts.format.addCommas(params.value[0].toLocaleString()))
      }
    "),
    textStyle = list(overflow = "break", fontSize = 11)
  ...
  e_legend(show = FALSE) %>%
  e_title("Top 200 Trigram Frequencies of Chief Complaint Parsed") %>%
  e_grid(containLabel = TRUE)
```
...
unnested_dd_trigrams <- dd_tokens %>%
  ...
  .[, c("word1", "word2", "word3") := tstrsplit(trigram, " ", fixed = TRUE)] %>%
  .[, trigram := fcase(
    word3 < word1 & word1 < word2, paste(word3, word1, word2),
    word3 < word2 & word2 < word1, paste(word3, word2, word1),
    word2 < word1 & word1 < word3, paste(word2, word1, word3),
    word2 < word3 & word3 < word1, paste(word2, word3, word1),
    word1 < word2 & word2 < word3, paste(word1, word2, word3),
    word1 < word2 & word3 < word2, paste(word1, word3, word2)
  )]
...
ddngram %>%
  ...
  mutate(
    code1 = str_wrap(paste(code1, description1, sep = "-"), 90),
    code2 = str_wrap(paste(code2, description2, sep = "-"), 90),
    code3 = str_wrap(paste(code3, description3, sep = "-"), 90),
    model = paste(code1, code2, code3, sep = "_"),
    model = gsub("\n", "<br />", model)
  ) %>%
  e_charts(trigram) %>%
  e_bar(n, name = "Frequency", color = "#005EAA", bind = model) %>%
...
Discharge Diagnosis Co-occurrence {data-navmenu="Term Network Graphs"} 
=======================================================================

### Discharge Diagnosis Parsed Code Co-occurrence Network Graph

```{r DD network, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 15, fig.height = 10}
dd_unigram_dfm <- dd_tokens %>%
  tokens_ngrams(n = 1) %>%
  dfm(tolower = FALSE) %>%
  dfm_trim(min_termfreq = 0.2)

dd_fcm <- fcm(dd_unigram_dfm)
...
ccdd_unigram_dfm <- ccdd_tokens %>%
  tokens_ngrams(n = 1) %>%
  dfm(tolower = FALSE) %>%
  dfm_trim(min_termfreq = 0.2)

ccdd_fcm <- fcm(ccdd_unigram_dfm)
...
```{r CC correlation network, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 10, results = "asis"}
...
if (nrow(check_cor) > 10) {
  ...
  if (nrow(cc_cor) == 0) {
    ...
    visNetwork(
      ...
      width = "100%",
      main = list(
        text = "Pairs of terms with at least a 0.45 correlation of appearing within the same chief complaint. Consecutive term pairs/bigrams removed.",
...
```{r search table, echo = FALSE, warning = FALSE, message = FALSE, results = "asis"}
if (nrow(check_cor) > 10 & nrow(cc_cor) > 0) {
  cc_cor %>%
    select(
      `Term 1` = item1,
      `Term 2` = item2,
...
```{r DD correlation network, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 10, results = "asis"}
...
if (nrow(check_cor) > 10) {
  ...
  visNetwork(
    ...
    width = "100%",
    main = list(
      text = "Pairs of codes with at least a 0.45 correlation of appearing within the same Discharge Diagnosis.",
      style = "font-family:Arial;font-size:15px;font-weight:bold;text-align:center"
    )
  ) %>%
    visPhysics(solver = "forceAtlas2Based") %>%
...
```{r DD search table, echo = FALSE, warning = FALSE, message = FALSE, results = "asis"}
if (nrow(check_cor) > 10) {
  dd_cor %>%
...
```{r CC increasing unigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht1, results = "asis"}
if (nrow(cc_unigrams_increasing) > 0) {
  tab_gg <- cc_unigrams_increasing %>%
    ...
    mutate(trend = map(.x = data, .f = function(.x) {
      ggplot(data = .x) +
        geom_line(aes(x = time_floor, y = frequency), color = "#005EAA", linewidth = 1.0) +
  ...
  ft_gg <- tab_gg %>%
    ...
    mk_par(j = "trend", value = as_paragraph(
      gg_chunk(value = trend, width = 5, height = 2)
    )) %>%
    theme_box() %>%
    border_outer(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    fix_border_issues() %>%
    vline(j = 1:3, border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border_inner_h(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "header") %>%
...
```{r DD increasing unigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht1, results = "asis"}
if (nrow(dd_unigrams_increasing) > 0) {
  ...
  ft_gg <- tab_gg %>%
    ...
    border_outer(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    fix_border_issues() %>%
    vline(j = 1:4, border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border_inner_h(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "header") %>%
    colformat_double(digits = 2) %>%
    font(fontname = "Source Sans Pro", part = "all") %>%
...
```{r CC decreasing unigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht2, results = "asis"}
if (nrow(cc_unigrams_decreasing) > 0) {
  tab_gg <- cc_unigrams_decreasing %>%
    ...
    mutate(trend = map(.x = data, .f = function(.x) {
      ggplot(data = .x) +
        geom_line(aes(x = time_floor, y = frequency), color = "#005EAA", size = 1.0) +
        scale_y_continuous(limits = c(0, NA)) +
        theme_minimal() +
        labs(
          x = "",
...
```{r DD decreasing unigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht2, results = "asis"}
...
```{r CC increasing bigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht3, results = "asis"}
...
```{r DD increasing bigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht3, results = "asis"}
if (nrow(dd_bigrams_increasing) > 0) {
  ...
  ft_gg <- tab_gg %>%
    ...
    border_outer(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    fix_border_issues() %>%
    vline(j = 1:5, border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border_inner_h(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "body") %>%
    border(border = officer::fp_border(color = "#E0E0E0", width = 1.2), part = "header") %>%
    colformat_double(digits = 2) %>%
    font(fontname = "Source Sans Pro", part = "all") %>%
...
```{r CC decreasing bigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht4, results = "asis"}
...
```{r DD decreasing bigram, echo = FALSE, message = FALSE, warning = FALSE, fig.height = ht4, results = "asis"}
