...
if (nrow(cc_alerts_unigram) > 0) {
  cc_trends_unigram <- cc_unigram_freq %>%
    ...
    left_join(
      res_out_ccunigram %>%
        .[, c("unigram", "date_test", "p.value", "significant", "scale")],
      by = c("feature" = "unigram", "date" = "date_test")
    ) %>%
    mutate(
...
if (nrow(cc_alerts_bigram) > 0) {
  cc_trends_bigram <- cc_bigram_freq %>%
    ...
    left_join(
      res_out_ccbigram %>%
        .[, c("bigram", "date_test", "p.value", "significant", "scale")],
      by = c("feature" = "bigram", "date" = "date_test")
    ) %>%
    mutate(
      p.value = ifelse(is.na(p.value), 0.5, p.value),
      significant = ifelse(is.na(significant), FALSE, significant)
