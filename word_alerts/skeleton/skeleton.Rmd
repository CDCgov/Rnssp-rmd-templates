...
params:
  ...
  definition:
    ...
    input: select
    choices: !r fl <- tempfile(fileext =".rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "word_alerts", "skeleton", "syndef.rds"), destfile = fl); definitions <- tibble::add_row(readRDS(fl), syndrome = "None (Custom CCDD Query)", query_logic = "", .before = 1); dplyr::pull(definitions, syndrome)
  ccdd_query: 
...
limit_ed <- ifelse(params$has_been_E, "&hasBeenE=1", "")
...
```{r parameter tab, echo = FALSE, warning = FALSE, message = FALSE}
...
param_df <- tibble(
  ...
  mutate(
    ...
    `Complex Query` = ifelse(complex_query_reset, change_dates(`Complex Query`, start_date, end_date), `Complex Query`)
  ) %>%
...
stopterms <- bind_rows(
  ...
  filter(!word %in% stopterms_to_remove)
...
results <- foreach(i = 1:length(dates), .combine = "bind_rows", .inorder = FALSE, .multicombine = TRUE) %dopar% {
  ...
  erbase <- tbl(con, "View_Cache_ER_Base_for_CCQV")

  current_date <- as.Date(dates[[i]])

  sample <- if (definition_pattern == "Syndrome") {
    
    if (params$has_been_E) {
      erbase %>%
        filter(Date == current_date, Category_flat %like% paste0("%;", definition_string, ";%"), hasBeenE == 1) %>%
        collect()
    } else {
      erbase %>%
    ...
    if (params$has_been_E) {
      erbase %>%
        filter(Date == current_date, SubCategory_flat %like% paste0("%;", definition_string, ";%"), hasBeenE == 1) %>%
        collect()
    } else {
      erbase %>%
    ...
    if (params$has_been_E) {
      erbase %>%
        filter(Date == current_date, CCDDCategory_flat %like% paste0("%;", definition_string, ";%"), hasBeenE == 1) %>%
        collect()
    } else {
      erbase %>%
...
date_range <- paste(format(min(df_dd$date), "%B %d, %Y"), "to", format(max(df_dd$date), "%B %d, %Y"))

n_days <- length(unique(df_dd$date))
...
if (nchar(params$complex_query) > 0) {
  url <- change_dates(params$complex_query, start_date, end_date)

  if (!grepl("&field=", url)) {
    url <- paste0(url, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD")
  }

  definition <- "Complex Query"
} else {
  ...
  if (unique(length(params$age_groups) == 1 & params$age_groups == "All Ages")) {
    if (params$site == "All") {
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?endDate=", end_date_api, "&percentParam=noPercent&datasource=va_hosp&startDate=", start_date_api, "&medicalGroupingSystem=", grouping_system, "&aqtTarget=DataDetails", definition_type, definition_string, "&geographySystem=hospitalregion&detector=nodetectordetector&timeResolution=daily", limit_ed, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD")
    } else {
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?endDate=", end_date_api, "&percentParam=noPercent&datasource=va_hosp&startDate=", start_date_api, "&medicalGroupingSystem=", grouping_system, "&site=", site_id, "&aqtTarget=DataDetails", definition_type, definition_string, "&geographySystem=hospitalregion&detector=nodetectordetector&timeResolution=daily", limit_ed, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD")
    }
  } else {
    ...
    if (params$site == "All") {
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?endDate=", end_date_api, "&percentParam=noPercent&datasource=va_hosp&startDate=", start_date_api, "&medicalGroupingSystem=", grouping_system, "&aqtTarget=DataDetails", definition_type, definition_string, "&geographySystem=hospitalregion&detector=nodetectordetector&timeResolution=daily", limit_ed, "&", age_group_string, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD&field=Age")
    } else {
      url <- paste0("https://essence2.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?endDate=", end_date_api, "&percentParam=noPercent&datasource=va_hosp&startDate=", start_date_api, "&medicalGroupingSystem=", grouping_system, "&site=", site_id, "&aqtTarget=DataDetails", definition_type, definition_string, "&geographySystem=hospitalregion&detector=nodetectordetector&timeResolution=daily", limit_ed, "&", age_group_string, "&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD&field=Age")
    }
  }
}
...
data_source <- "NSSP-ESSENCE Facility Location (Full Details)"
...
ggplot(data = volume) +
  geom_line(aes(x = date, y = n), linewidth = 1.0, color = "#00928F") +
...
Similar to chief complaints, the discharge diagnosis parsed field **(added in August 2022)** can be tokenized into bigrams.The ESSENCE discharge diagnosis parser includes

- Parsing of the original discharge diagnosis into 3 separate fields for valid matches to regular expressions for ICD-9, ICD-10, or SNOMED codes (C_DiagnosisCode_ICD9_Flat, C_DiagnosisCode_ICD10_Flat, and C_DiagnosisCode_SNOMED_Flat). This step removes text and code descriptions. Keep in mind that this **does not** remove invalid diagnosis codes that still successfully match to the regular expressions used.
- Combining these 3 fields into DDParsed

Additional template-level processing of the discharge diagnosis parsed includes

- Replacement of semicolon or alternative potential separators between discharge diagnosis codes with a single space
- Removal of duplicate diagnostic codes in the same discharge diagnosis parsed
...
### **ESSENCE Data Details URLs**

When the Facility Location (Full Details) data source is used, the chief complaint and discharge diagnosis unigram and bigram tables below will contain ESSENCE URLs linking to line-level records associated with alerts. The date of the records should align with the date listed under the Test Date column. Data details URLs are not available when the CCQV Datamart Backup data source is used as the daily date field is excluded from the CCQV data source within the ESSENCE UI. 

***Please note that the number of records displayed in the ESSENCE UI for a chief complaint bigram query may slightly differ from the frequency reported in the tables within this report due to stop word removal.*** As an example, this template will transform chief complaints of "SHORTNESS OF BREATH" to "SHORTNESS BREATH" and generate an ESSENCE data details query for !SHORTNESS BREATH! to be run on the CCDD Parsed field. The records with an original parsed chief complaint of "SHORTNESS OF BREATH" will be included in the frequency reported in the table but will not appear in the ESSENCE UI. Manual updates to the query run by the URL may need to be modified in ESSENCE configuration options to account for these variations. 

For discharge diagnosis bigrams, the ESSENCE data details URLs will account for permutations of two diagnosis codes. As mentioned previously, this template orders discharge diagnosis bigrams alphanumerically so that permutations of two diagnosis codes are counted together. As an example, bigrams "E876 R410" and "R410 E876" are considered equivalent and will therefore be reported as "E876 R410" with the combined frequencies. The ESSENCE data details URLs will run queries that search CCDD Parsed for both variations (i.e., query CCDD Parsed for  \\^;E876;R410;^,OR,^;R410;E876;\\^). 

## **Chief Complaint Term Alerts**

```{r process blocks CC, echo = FALSE, warning = FALSE, message = FALSE, output = FALSE}

dates <- seq.Date(from = as.Date(min(df_dd$date)), to = as.Date(max(df_dd$date)), by = "1 day")
...
date_breaks <- ceiling(as.integer(difftime(max_date, min_date, units = "days")) / 7)
date_seq <- seq.Date(min_date, max_date, by = date_breaks)

plotcc_unigram <- ggplot(data = daily_cc_alerts_unigram) +
  geom_col(aes(x = date_test, y = n), fill = "#005DAA") +
  scale_y_continuous(
  ...
  scale_x_date(breaks = date_seq, date_labels = "%b %d") +
  theme_minimal() +
  labs(
    x = "Date",
...
plotdd_unigram <- ggplot(data = daily_dd_alerts_unigram) +
  geom_col(aes(x = date_test, y = n), fill = "#00928F") +
  scale_y_continuous(
...
daily_alerts_combined <- daily_cc_alerts_unigram %>%
  rbind(daily_cc_alerts_bigram) %>%
  rbind(daily_dd_alerts_unigram) %>%
  rbind(daily_dd_alerts_bigram) %>%
  mutate(type = str_wrap(type, 20))

pal <- c("#005DAA", "#005DAA", "#00928F", "#00928F")
  ...
  if (length(unique(unigram_dist$date)) > 1) {
    unigram_dist %>%
      ...
      add_markers(
        ...
        mode = "markers",
        size = ~ 2 * radius,
        marker = list(symbol = "circle", sizemode = "diameter", color = "#80AED4", line = list(color = "#005DAA", width = 2)),
        showlegend = FALSE,
        text = ~ paste(
          "<br>Date:</b>", date,
...
if (nrow(cc_alerts_unigram) > 0) {
  ...
  ccunigram_table <- cc_trends_unigram %>%
    ...
    mutate(
      ...
      sparkline = NA,
      dd_url = url,
      dd_url = gsub("/api/dataDetails/csv", "/servlet/DataDetailsServlet", dd_url),
      dd_url = str_replace(dd_url, "startDate=\\d{1,2}\\w{3}\\d{4}", paste0("startDate=", format(date_test, "%d%b%Y"))),
      dd_url = str_replace(dd_url, "endDate=\\d{1,2}\\w{3}\\d{4}", paste0("endDate=", format(date_test, "%d%b%Y"))),
      dd_url = gsub("&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD", "", dd_url),
      dd_url = gsub("&field=Age", "", dd_url), 
      dd_url = paste0(dd_url, "&CCDDParsedFreeText=!", feature, "!")
    ) %>%
  ...
  tbl <- ccunigram_table %>%
    reactable(
      ...
      columns = list(
          ...
          cell = function(value, index) {
            ...
            .fig <- plot_ly(.dat) %>%
              add_trace(
                x = ~date,
                y = ~prop,
                line = list(color = "#005DAA", width = 1),
                fillcolor = "#80AED4",
                fill = "tozeroy",
                mode = "lines",
                type = "scatter",
        ...
        dd_url = colDef(
          name = "ESSENCE Data Details URL", 
          cell = function(value) {
            
            if (ccqv_param_reset) {
              "Not available for CCQV"
            } else {
...
if (nrow(cc_alerts_bigram) > 0) {
  ...
  ccbigram_table <- cc_trends_bigram %>%
    ...
    mutate(
      ...
      dd_url = gsub("&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD", "", dd_url),
      dd_url = gsub("&field=Age", "", dd_url), 
      dd_url = paste0(dd_url, "&CCDDParsedFreeText=!", gsub(" ", "%20", feature), "!")
    ) %>%
    arrange(date_test, feature)
...
if (nrow(dd_alerts_unigram) > 0) {
  ...
  ddunigram_table <- dd_trends_unigram %>%
    ...
    mutate(
      ...
      dd_url = gsub("&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD", "", dd_url),
      dd_url = gsub("&field=Age", "", dd_url), 
      dd_url = paste0(dd_url, "&CCDDParsedFreeText=%5E;", feature, ";%5E")
    ) %>%
          ...
          cell = function(value, index) {
            ...
            .fig <- plot_ly(.dat) %>%
              add_trace(
                x = ~date,
                y = ~prop,
                line = list(color = "#00928F", width = 1),
                fillcolor = "#4EBAAA",
                fill = "tozeroy",
                mode = "lines",
                type = "scatter",
...
if (nrow(dd_alerts_bigram) > 0) {
  ...
  ddbigram_table <- dd_trends_bigram %>%
    ...
    mutate(
      ...
      dd_url = gsub("&field=Date&field=WeekYear&field=ChiefComplaintOrig&field=ChiefComplaintParsed&field=DischargeDiagnosis&field=DDParsed&field=CCDD", "", dd_url),
      dd_url = gsub("&field=Age", "", dd_url), 
      dd_url = paste0(dd_url, "&CCDDParsedFreeText=%5E;", code1, " ", code2, ";%5E,OR,%5E;", code2, " ", code1, ";%5E"), 
      dd_url = gsub(" ", ";", dd_url)
    ) %>%
    left_join(icd_codes, by = c("code1" = "code")) %>%
    rename(description1 = description) %>%
    ...
    arrange(date_test, feature) %>%
    mutate(
      description1 = paste(code1, description1), 
      description2 = paste(code2, description2),
      description1 = gsub("ICD-10", "(ICD-10)", description1),
      description2 = gsub("ICD-10", "(ICD-10)", description2),
      description1 = gsub("ICD-9", "(ICD-9)", description1),
      description2 = gsub("ICD-9", "(ICD-9)", description2)
    ) %>%
    select(
      -code1,
      -code2
