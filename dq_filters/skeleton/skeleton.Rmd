...
dq_distribution_function <- function(year_to_plot) {
  ...
  output_title <- ggdraw() + 
    draw_label("National Distribution of Data Quality Filters", fontface = "bold", size = 14)
...
ddi_distributions <- dq_volume_data %>%
  ggplot(aes(x = dq_year, y = ddi)) +
...
cov_distributions <- dq_volume_data %>%
  ggplot(aes(x = dq_year, y = cov)) +
  geom_boxplot(aes(fill = dq_year), outlier.shape = 21) +
  theme_bw() +
  theme(
...
dq_table = facility_dq_data %>% mutate(across(where(is.numeric), round, 2))
