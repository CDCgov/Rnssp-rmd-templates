...
if (as.character(str_match(params$definition[3], pattern = "CCDD Category|Subsyndrome|Syndrome")) == "Syndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params$definition[3]), "syndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "essencesyndromes"
}
...
```{r def2_elements, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=length(params$definition) >= 2}
...
```{r def3_elements, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=length(params$definition) > 2}
