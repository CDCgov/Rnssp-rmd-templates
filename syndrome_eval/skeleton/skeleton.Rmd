...
params:
  ...
  has_been_E: 
    ...
    value: true
  chunk_pull: 
    label: "Chunk data pull (per day)?"
    value: false
  definition: 
    label: "Grouping systems (Select up to 3):" 
    ...
    multiple: yes
  custom_query1: 
    label: "Custom Query1 (Will Overwrite first CCDD definition):"
    value: ""
    input: text
    placeholder: "Free Text Query or Data Details CSV URL (overwrites previous inputs except Dates)"
  custom_query_label1:
    label: "Enter label for Custom Query1"
    value: ""
    input: text
    placeholder: "Influenza"
  custom_query2: 
    label: "Custom Query2 (Will Overwrite second CCDD definition):"
    value: ""
    input: text
    placeholder: "Free Text Query or Data Details CSV URL (overwrites previous inputs except Dates)"
  custom_query_label2:
    label: "Enter label for Custom Query2"
    value: ""
    input: text
    placeholder: "Influenza"
  custom_query3: 
    label: "Custom Query3 (Will Overwrite third CCDD definition):"
    value: ""
    input: text
    placeholder: "Free Text Query or Data Details CSV URL (overwrites previous inputs except Dates)"
  custom_query_label3:
    label: "Enter label for Custom Query3"
    value: ""
    input: text
    placeholder: "Influenza"
  doc_title:
    label: "Title:"
    value: Syndrome Definition Evaluation
    input: text
  data_export: 
...
```{r free_text_checks, echo=FALSE, message=FALSE, warning=FALSE}
params2 <- params
custom_query_label <- params$definition
custom_query <- params$definition

if(nchar(params$custom_query_label1) > 0){
  custom_query_label[1] <- params$custom_query_label1
}

if(nchar(params$custom_query_label2) > 0){
  custom_query_label[2] <- params$custom_query_label2
}

if(nchar(params$custom_query_label3) > 0){
  custom_query_label[3] <- params$custom_query_label3
}

if(nchar(params$custom_query1) > 0){
  params2$definition[1] <- custom_query_label[1] %>% 
    paste0("Custom Query: ", .)
  custom_query[1] <- params$custom_query1
}

if(nchar(params$custom_query2) > 0){
  params2$definition[2] <- custom_query_label[2] %>% 
    paste0("Custom Query: ", .)
  custom_query[2] <- params$custom_query2
}

if(nchar(params$custom_query3) > 0){
  params2$definition[3] <- custom_query_label[3] %>% 
    paste0("Custom Query: ", .)
  custom_query[3] <- params$custom_query3
}

params2$definition <- params2$definition[!is.na(params2$definition)]
custom_query <- custom_query[!is.na(custom_query)]
```
...
hasBeenE <- ifelse(params$has_been_E, "&hasBeenE=1", "")
...
if (as.character(str_match(params2$definition[1], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "CCDD Category") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[1]), "ccdd category: "), " ", "%20")
  definition_type <- "&ccddCategory="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[1], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Subsyndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[1]), "subsyndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "chiefcomplaintsubsyndromes"
}

if (as.character(str_match(params2$definition[1], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Syndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[1]), "syndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[1], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Custom Query") {
  definition_string <- str_replace_all(custom_query[1], "\\^", "%5E") %>%
    str_replace_all(" ", "%20") %>%
    str_replace_all("\\[", "%5B") %>%
    str_replace_all("\\]", "%5D")
...
if(is_valid_url(params2$custom_query1)){
  url <- paste0(params2$custom_query1, "&field=", fields) %>% 
    change_dates(query_start, query_end)
}
...
def1_name <- params2$definition[1]
...
```{r def2_url, echo=FALSE, message = FALSE, warning = FALSE, results = 'hide', eval=length(params2$definition) >= 2}

if (as.character(str_match(params2$definition[2], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "CCDD Category") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[2]), "ccdd category: "), " ", "%20")
  definition_type <- "&ccddCategory="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[2], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Subsyndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[2]), "subsyndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "chiefcomplaintsubsyndromes"
}

if (as.character(str_match(params2$definition[2], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Syndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[2]), "syndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[2], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Custom Query") {
  definition_string <- str_replace_all(custom_query[2], "\\^", "%5E") %>%
    str_replace_all(" ", "%20") %>%
    str_replace_all("\\[", "%5B") %>%
    str_replace_all("\\]", "%5D")
  definition_type <- "&ccddFreeText="
...
if(is_valid_url(params2$custom_query2)){
  url2 <- paste0(params2$custom_query2, "&field=", fields) %>% 
    change_dates(query_start, query_end)
}
```

```{r def2_setup, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) >= 2}
def2_name <- params2$definition[2]
...
```{r def3_url, echo=FALSE, message = FALSE, warning = FALSE, results = 'hide', eval=length(params2$definition) > 2}

if (as.character(str_match(params2$definition[3], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "CCDD Category") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[3]), "ccdd category: "), " ", "%20")
  definition_type <- "&ccddCategory="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[3], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Subsyndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[3]), "subsyndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "chiefcomplaintsubsyndromes"
}

if (as.character(str_match(params2$definition[3], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Syndrome") {
  definition_string <- str_replace_all(str_remove_all(tolower(params2$definition[3]), "syndrome: "), " ", "%20")
  definition_type <- "&medicalGrouping="
  grouping_system <- "essencesyndromes"
}

if (as.character(str_match(params2$definition[3], pattern = "CCDD Category|Subsyndrome|Syndrome|Custom Query")) == "Custom Query") {
  definition_string <- str_replace_all(custom_query[3], "\\^", "%5E") %>%
    str_replace_all(" ", "%20") %>%
    str_replace_all("\\[", "%5B") %>%
    str_replace_all("\\]", "%5D")
  definition_type <- "&ccddFreeText="
...
if(is_valid_url(params2$custom_query3)){
  url3 <- paste0(params2$custom_query3, "&field=", fields) %>% 
    change_dates(query_start, query_end)
}
```

```{r def3_setup, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}

def3_name <- params2$definition[3]
...
if(params$chunk_pull){
  def1_full <- try(
    return_longterm_query(def1_url, profile = myProfile, params$start_date, params$end_date, by = 1),
    silent = TRUE
  )
} else {
  def1_full <- try(
    myProfile$get_api_data(def1_url, fromCSV = TRUE, show_col_types = FALSE),
    silent = TRUE
  )
}

premature_quit <- any(all(class(def1_full) == "try-error"), all(dim(def1_full) == c(0, 0)))
...
knitr::knit_exit("Render ends prematurely.
                 The Data pull fails! Please, Check your User Credentials!")
```
...
```{r def2_extract, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) >= 2}
if(params$chunk_pull){
  def2_full <- return_longterm_query(def2_url, profile = myProfile, params$start_date, params$end_date, by = 1)
} else {
  def2_full <- myProfile$get_api_data(def2_url, fromCSV = TRUE, show_col_types = FALSE)
}
...
```{r def3_extract, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}

if(params$chunk_pull){
  def3_full <- return_longterm_query(def3_url, profile = myProfile, params$start_date, params$end_date, by = 1)
} else {
  def3_full <- myProfile$get_api_data(def3_url, fromCSV = TRUE, show_col_types = FALSE)
}
...
```{r def1_elements, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r def2_elements, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=length(params2$definition) >= 2}
...
```{r def3_elements, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=length(params2$definition) > 2}
...
if (length(params2$definition) == 2) {
...
```{r title_oneDef, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) == 1}
# cat(
paste0(
  "
...
```{r two_def_info, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) >= 2}
# cat(
paste0(
  "
...
```{r def1_table, echo=FALSE, message=FALSE, warning=FALSE, eval=!is_valid_url(params2$custom_query1)}
...
```{r def2_name_info, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) >= 2}
paste0(
  "
...
```{r def2_table, echo=FALSE, message=FALSE, warning=FALSE, eval=(length(params2$definition) >= 2 & !is_valid_url(params2$custom_query2))}
...
```{r def3_name_info, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 2}
paste0(
  "
...
```{r def3_table, echo=FALSE, message=FALSE, warning=FALSE, eval=(length(params2$definition) > 2 & !is_valid_url(params2$custom_query3))}
...
```{r oneDef_visits_over_time, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) == 1}
cat("
...
```{r time_series_def2, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) >= 2}
...
```{r time_series_def3, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r time_series, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 1}
plot_ly() %>%
  add_lines(
...
```{r same_scale, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r time_series_twoDef, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 2}
plot_ly() %>%
  add_lines(
    data = time_series_def1,
    x = ~Date, y = ~Visits, name = ~Syndrome,
...
```{r time_series_threeDef, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
plot_ly() %>%
  add_lines(
    data = time_series_def1,
    x = ~Date, y = ~Visits, name = ~Syndrome,
...
```{r independent_scale, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r ind_scale_twoDef, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 2}
...
```{r ind_scale_threeDef, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
ts_plot_def1 <- plot_ly() %>%
  add_lines(
    data = time_series_def1,
    x = ~Date, y = ~Visits, name = ~Syndrome,
...
```{r two_defs_prep, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 2}
...
```{r three_defs_prep, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r all_twodef_visits, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) == 2}
cat(
  paste0("
...
```{r all_threedef_visits, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 2}
cat(
  paste0("
...
```{r twodef_summary, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2, eval=length(params2$definition) == 2}
...
```{r threedef_summary, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, eval=length(params2$definition) > 2}
...
```{r percent_all_def_visits, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r two_def_comparison, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 2}
...
```{r three_def_comparison, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r overlap_not_to_scale, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r two_def_venn, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, eval=length(params2$definition) == 2}
...
```{r three_def_venn, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r overlap_to_scale, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r two_def_euler, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, eval=length(params2$definition) == 2}
...
```{r three_def_euler, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r upset_plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', eval=length(params2$definition) > 1}
cat("
...
```{r two_def_upset, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, eval=length(params2$definition) == 2}
...
```{r three_def_upset, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2}
...
```{r onedef_export, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 1 & params$data_export}
...
```{r twodef_export, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) == 2 & params$data_export}
# All Visits
write.csv(
...
```{r threedef_export, echo=FALSE, message=FALSE, warning=FALSE, eval=length(params2$definition) > 2 & params$data_export}
...
___
*The template for this report was originally created by [Sara Chronister](https://github.com/sara-chronister), consolidated, improved, and adapted to the Rnssp package by [Gbedegnon Roseric Azondekon](https://rosericazondekon.github.io). For questions, ideas for improvement/collaboration, or attribution, please submit an issue at <https://github.com/CDCgov/Rnssp-rmd-templates/issues>.*
