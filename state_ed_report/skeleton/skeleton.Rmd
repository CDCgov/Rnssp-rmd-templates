...
src.result[, status := fcase(
  p.value < 0.01 & statistic > 0, "Significantly Increasing",
...
res <- cbind(dt[, .SD, .SDcols = c("date")], shifted.numerator, shifted.denominator) %>%
  melt(
    id = c("date", "county", "ccdd_category"),
    measure = patterns("pos", "neg"),
    value.name = c("pos", "neg"),
    variable.name = "date_num"
  ) %>%
...
for (i in 1:nrow(all.county.plots)) {
  grid <- ggarrange(
    plotlist = c(all.county.plots[i, 2], all.county.plots[i, 3], all.county.plots[i, 4]),
    ncol = 3, nrow = 1,
    common.legend = TRUE, legend = "top"
  )
  annotated.grid <- annotate_figure(grid,
    top = text_grob(paste0(
      all.county.plots[i, 5], " County: ",
