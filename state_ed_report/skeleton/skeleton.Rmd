...
params:
  ...
  ccdd_categories:
    ...
    value: !r fl <- tempfile(fileext =".rds"); download.file(file.path("https://raw.githubusercontent.com", "cdcgov", "Rnssp-rmd-templates", "master", "text_mining", "skeleton", "syndef.rds"), destfile = fl); head(dplyr::pull(readRDS(fl), syndrome), 2)
    choices: !r library(magrittr); read.csv("ccdd_categories.csv") %>% dplyr::pull(ccdd_category)
    input: select
    multiple: yes
  start_date:
