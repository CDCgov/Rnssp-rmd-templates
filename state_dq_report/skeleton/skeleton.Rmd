...
description: "This is a site-specific data quality report that generates coverage, timeliness, completeness, and validity metrics similar to the NSSP Data Quality dashboards. It utilizes your site's BioSense Production Processed table and the ESSENCE API to assess the data quality of patient encounters that occurred during the chosen date range and match the selected C_Patient_Class. Please knit with parameters within the NSSP RStudio Workbench. The default end_date parameter reflects the recommended lag time of at least two days. Caution: selecting a date range longer than 90 days will result in extensive rendering time."
output:
  html_document:
    toc: true
params:
...
```{r helpers, echo = FALSE, message = FALSE, warning = FALSE, include = FALSE}
...
pat.class <- if (params$patient_class == "E (Emergency Department)") {
...
patient_class <- if (params$patient_class == "E (Emergency Department)") {
  paste0("E")
} else if (params$patient_class == "I (Inpatient)") {
  paste0("I")
} else if (params$patient_class == "O (Outpatient)") {
...
operational_crosswalk <- RODBC::sqlQuery(
  con,
  paste0(
...
active_crosswalk <- active_crosswalk %>%
  dplyr::left_join(., MFT, by = "C_Biosense_Facility_ID") %>%
  dplyr::distinct() %>%
...
plotly::plot_ly(
  ...
  plotly::layout(
    title = "Total Syndromic Messages Received by Day",
    yaxis = aa,
    xaxis = list(title = "Date")
  )
```
...
lag.table <- lag.table %>%
  ...
  mutate(
    `Timeliness Category` = fct_relevel(
      `Timeliness Category`,
      c(">48 Hours", "24-48 Hours", "<24 Hours")
    )
  )
...
delay.plot$`Facility Name` <-
  factor(
    delay.plot$`Facility Name`,
    levels = unique(delay.plot$`Facility Name`)[
      order(delay.plot$`Facility Name`, decreasing = TRUE)
    ]
  )
...
FacLag$`Facility Name` <- factor(
  FacLag$`Facility Name`,
  levels = unique(FacLag$`Facility Name`)[
    order(FacLag$`Facility Name`, decreasing = TRUE)
  ]
)
...
lagsummary %>%
  datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: center; color:black; font-size:125% ;",
...
CClag %>%
  datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: center; color:black; font-size:125% ;", 
      "Chief Complaint Timeliness"
    )
  ) %>%
  formatStyle(
    "CClagavg", color = styleInterval(c(24, 48), c("green", "blue", "red"))
  ) %>%
  formatStyle(
    "CClagmedian", color = styleInterval(c(24, 48), c("green", "blue", "red"))
  )
```
...
DXlag %>%
  ...
  formatStyle(
    "DXlagavg", color = styleInterval(c(24, 48), c("green", "blue", "red"))
  ) %>%
  formatStyle(
    "DXlagmedian", color = styleInterval(c(24, 48), c("green", "blue", "red"))
  )
```
...
element1.summary <- messages %>%
  ...
  dplyr::mutate(
    ...
    CDthstatus = case_when(
      any(!is.na(C_Death)) ~ "complete",
      all(is.na(C_Death)) ~ "null"
    ),
    CFacIDstatus = case_when(
      any(!is.na(C_Facility_ID)) ~ "complete",
    ...
    CPatClstatus = case_when(
      any(!is.na(C_Patient_Class)) ~ "complete",
      all(is.na(C_Patient_Class)) ~ "null"
    ),
    CPatIDstatus = case_when(
      any(!is.na(C_Unique_Patient_ID)) ~ "complete",
    ...
    ProcIDstatus = case_when(
      any(!is.na(Processing_ID)) ~ "complete",
...
element1.sum <- element1.summary %>%
  ...
  summarise(
    ...
    Sending_Facility_ID = round(
      (sum(SFIDstatus == "complete") /
         (sum(SFIDstatus == "complete") +
            sum(SFIDstatus == "null")) * 100), 2
    ),
    Treating_Facility_ID = round(
...
element2.summary <- messages %>%
  ...
  dplyr::mutate(
    ...
    MDTstatus = case_when(
      any(!is.na(Message_Date_Time)) ~ "complete",
...
element1.plot %>%
  ggplot(aes(x = fct_rev(Metric), y = Percent_Complete, fill = Percent_Complete)) +
  geom_bar(position = "dodge", stat = "identity", color = "black") +
  geom_shadowtext(
    aes(label = scales::percent((Percent_Complete / 100), accuracy = 1L)),
  ...
  scale_fill_gradient2(
    ...
    low = "red", mid = "yellow", high = "#2ca02c",
    midpoint = 50
  ) +
  coord_flip() +
  theme_minimal() +
...
element2.plot %>%
  ggplot(
    aes(x = fct_rev(Metric), y = Percent_Complete, fill = Percent_Complete)
  ) +
  geom_bar(position = "dodge", stat = "identity", color = "black") +
  geom_shadowtext(
    aes(label = scales::percent((Percent_Complete / 100), accuracy = 1L)),
...
completeness.date <- paste(
  format(min(cc.available$date), "%B %d, %Y,"),
  "to",
  format(max(cc.available$date), "%B %d, %Y")
)

plot.ccavailable <- cc.available %>%
  ggplot() +
...
plot.ddavailable <- ggplot(dd.available) +
  ...
  annotate(
    "text",
    x = min(dd.available$date) + days(7),
    y = 74.5,
    label = "NSSP Completeness Threshold"
  ) +
...
ccdx.summary.table %>%
  ...
  formatStyle(
    "CC.pct.complete",
    color = styleInterval(c(60, 80), c("red", "#f26a44", "green"))
  ) %>%
  formatStyle(
    "DX.pct.complete",
    color = styleInterval(c(60, 80), c("red", "#f26a44", "green"))
...
priority.1 %>%
  ...
  tab_footnote(
    ...
    locations = cells_column_labels(
      columns = "Code Set Location (PHIN-VADS) or Processing Logic"
    )
  )
...
p1_R <- messages %>%
  mutate(
    ...
    vFacility_Type_Code = case_when(
      str_detect(
        Facility_Type_Code, "261QE0002X|1021-5|261QM2500X|261QP2300X|261QU0200X"
      ) ~ "valid",
      TRUE ~ "invalid"
    ),
...
p2_RE <- messages %>%
  mutate(
    discharge_disposition = case_when(
      str_detect(
        Discharge_Disposition,
        "1-9|0[1-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9]|6[0-9]|7[0-9]|8[0-9]|9[0-9]|10[0-9]"
      ) ~ 1,
      TRUE ~ 0
    ),
    ...
    age_units_reported = case_when(
      str_detect(
        Age_Units_Reported,
        "(d|m|UNK|wk|a|day|DAY|month|MONTH|unknown|UNKNOWN|week|WEEK|year|YEAR|DAYS|WEEKS|MONTHS|YEARS|days|weeks|months|years)"
      ) ~ 1,
      TRUE ~ 0
    ),
    race_code = case_when(
      str_detect(
        Race_Code,
        regex(
          "(?>1002-5|2028-9|2054-5|2076-8|2131-1|2106-3|PHC1175|UNK|NI|ASKU|NASK)*?",
          ignore_case = TRUE
        )
      ) ~ 1,
      TRUE ~ 0
    ...
    ethnicity_code = case_when(
      str_detect(
        Ethnicity_Code, regex(
          "(2135-2|2186-5|PHC1175|UNK)",
          ignore_case = TRUE
        )
      ) ~ 1,
      TRUE ~ 0
    ...
    ethnicity_description = case_when(
      str_detect(
        Ethnicity_Description,
        "(Hispanic or Latino|Not Hispanic or Latino|HISPANIC OR LATINO|NOT HISPANIC OR LATINO|UNKNOWN|unknown)"
      ) ~ 1,
      TRUE ~ 0
    ),
...
priority_1_plot %>%
  ggplot(aes(x = fct_rev(Metric), y = Percent_Valid, fill = Percent_Valid)) +
  geom_bar(position = "dodge", stat = "identity", color = "black") +
  geom_shadowtext(
    aes(label = scales::percent((Percent_Valid / 100), accuracy = 1L)),
...
p1_facility <- p1_R_facility %>%
  left_join(., p1_RE_facility, by = "Facility_Name")
...
p2_facility <- p2_R_facility %>%
  left_join(., p2_RE_facility, by = "Facility_Name")

cap <- paste(
  "NSSP Priority 1 Element Percent Validity by Facility from",
  start_date_lab,
  "through",
  end_date_lab
)
...
p1_facility %>%
  ...
  row_spec(
    0,
    color = "white", background = "black", align = "c", font_size = 9
  ) %>%
  column_spec(1:22, border_left = TRUE, border_right = TRUE) %>%
  scroll_box(width = "100%", height = "600px")

cap1 <- paste(
  "NSSP Priority 2 Element Validity by Facility from",
  start_date_lab,
  "through",
  end_date_lab
)
...
p2_facility %>%
  ...
  row_spec(
    0, color = "white", background = "black", align = "c", font_size = 9
  ) %>%
...
informative.date <- paste(
  format(min(cc.informative$date), "%B %d, %Y,"),
  "to",
  format(max(cc.informative$date), "%B %d, %Y")
)

plot.ccinformative <- cc.informative %>%
  ggplot() +
  ...
  annotate(
    "text",
    x = min(cc.informative$date) + days(7),
    y = 74.5,
    label = "NSSP Validity Threshold"
  ) +
...
plot.ddinformative <- ggplot(dd.informative) +
  ...
  annotate(
    "text",
    x = min(dd.informative$date) + days(7),
    y = 74.5,
    label = "NSSP Validity Threshold"
  ) +
