...
# ICD-10 FY-2024 conversion file
code_updates <- readRDS("icd10_fy2024_conversion_ref.rds")

previous_codes <- code_updates %>%
  select(previous_clean) %>%
  distinct(previous_clean) %>%
  mutate(conversion_status = "Code revised for FY-2024")
...
new_codes <- code_updates %>%
  ...
  mutate(conversion_status = "Updated code for FY-2024")
...
dd_increase <- dd_significant %>%
  ...
  mutate(
    ...
    conversion_status = ifelse(is.na(conversion_status), "No revision detected for FY-2024", conversion_status),
    conversion_status = factor(conversion_status, levels = c("Updated code for FY-2024", "No revision detected for FY-2024"))
  ) %>%
  select(
    code,
    conversion_status,
...
dd_increase_shift <- dd_increase %>%
  filter(conversion_status == "Updated code for FY-2024") %>%
...
### **ICD-10 Codes with Significant Increase in Occurrence Due to Code Revisions for FY-2024**

The following table includes a subset of ICD-10 codes identified as having recent significant increases in usage due to ICD-10-CM revisions and updates for FY-2024. Updated 2024 ICD-10-CM codes are to be used for discharge diagnoses and patient encounters occurring between October 1, 2023 through September 30, 2024. These codes were identified by joining to a reference file derived from the 2024 Conversion Table published on the [CMS website](https://www.cms.gov/medicare/coding-billing/icd-10-codes/2024-icd-10-cm). This reference file contains the following fields: 
...
The sparklines in the following table represent trends for updated FY-2024 codes. The table below will display a message of "No rows found" in the event that 0 ICD-10 codes with significant increases in usage corresponding to FY-2024 code revisions are detected.
...
tbl_increase_detailed <- dd_increase %>%
  ...
  reactable(
    ...
    columns = list(
      ...
      code = colDef(
        name = "Updated ICD-10 Code for FY-2024",
        width = 100
      ),
...
div(
  class = "term-table",
  div(class = "term-table-title", "Increasing ICD-10 Diagnoses with Code Revisions for FY-2024"),
...
dd_decrease <- dd_significant %>%
  ...
  mutate(
    conversion_status = ifelse(is.na(conversion_status), "No revision detected for FY-2024", conversion_status),
    conversion_status = factor(conversion_status, levels = c("Code revised for FY-2024", "No revision detected for FY-2024"))
  ) %>%
  left_join(icd10_features, by = c("code", "description")) %>%
  mutate(chapter = paste(chapter, codes)) %>%
  select(-codes) %>%
...
dd_decrease_shift <- dd_decrease %>%
  filter(conversion_status == "Code revised for FY-2024") %>%
...
### **ICD-10 Codes with Significant Decrease in Occurrence Due to Code Revisions for FY-2024**

The following table includes a subset of ICD-10 codes identified as having recent significant decreases in usage due to ICD-10-CM revisions and updates for FY-2024. Updated 2024 ICD-10-CM codes are to be used for discharge diagnoses and patient encounters occurring between October 1, 2023 through September 30, 2024. These codes were identified by joining to a reference file derived from the 2024 Conversion Table published on the [CMS website](https://www.cms.gov/medicare/coding-billing/icd-10-codes/2024-icd-10-cm). This reference file contains the following fields: 
...
Keep in mind that previous codes may repeat over several rows for revisions that map to multiple options for FY-2024 discharge diagnoses. As an example, FY-2024 revisions for the D319 code for benign neoplasm of ill-defined sites within the digestive system include the following 2 options: 

  1. D1391: Familial adenomatous polyposis
  2. D1399: Benign neoplasm of ill-defined sites within the digestive system
  
The sparklines in the following table represent trends for the previous code and will therefore repeat for rows representing multiple conversion options. The table below will display a message of "No rows found" in the event that 0 ICD-10 codes with significant decreases in usage corresponding to FY-2024 code revisions are detected.
...
div(
  class = "term-table",
  div(class = "term-table-title", "Decreasing ICD-10 Diagnoses with Code Revisions for FY-2024"),
